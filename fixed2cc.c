...
convert (char* input_file, char* output_file)
{
  ...
  do
  {
    ...
    if (recl > 0)
    {
      ...
      if (written != '\n')
      {
        printf("fputc() did not write expected character (newline), but returned %d instead at record %d\n", written, rec_total);
      }
  ...
  printf("Copied %d records from %s to %s\n", rec_ok, input_file, output_file);
...
main (int argc, char** argv)
{
  if (argc < 3)
  {
    printf("usage: %s <record file> <output file>\nNOTE: only feed me fixed-length records, please\n", argv[0]);
    return 22;
  } else {
